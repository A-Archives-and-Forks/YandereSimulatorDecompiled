			...
			if (Student != null && Student.enabled && Student.DistractionSpot != new Vector3(base.transform.position.x, Student.transform.position.y, base.transform.position.z))
			{
				...
				if (!Radio)
				{
					if (Student != Originator)
					{
						...
						if ((!Student.TurnOffRadio && Student.Alive && !Student.Blind && !Student.Pushed && !Student.Dying && !Student.Alarmed && !Student.Guarding && !Student.Wet && !Student.Slave && !Student.CheckingNote && !Student.WitnessedMurder && !Student.WitnessedCorpse && !Student.Emetic && !Student.Confessing && !StudentIsBusy && !Student.FocusOnYandere && !Student.Fleeing && !Student.Shoving && !Student.SentHome && Student.ClubActivityPhase < 16 && !Student.Vomiting && !Student.Lethal && !Student.Headache && !Student.Sedated && !Student.SenpaiWitnessingRivalDie && !Student.Hunted && !Student.Drowned && !Student.DramaticReaction && !Student.Yandere.Chased && !Student.Hunting && !Student.ImmuneToLaughter && !Student.ListeningToReport && !Student.Distracted && !Student.RetrievingMedicine && !Student.Struggling) || (!Student.StudentManager.Eighties && Student.Persona == PersonaType.Protective && Originator != null && Originator.StudentID == 11 && !Student.Hunted && !Student.Emetic && !Student.Lethal && !Student.Headache && !Student.Slave && !Student.Sedated))
						{
