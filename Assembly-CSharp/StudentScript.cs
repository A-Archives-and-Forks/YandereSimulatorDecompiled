...
public class StudentScript : MonoBehaviour
{
	...
	public ScheduleBlock[] OriginalOriginalScheduleBlocks;
	...
	public bool RetrievingMedicine;
	...
	private SubtitleType LostPhoneSubtitleType
	{
		get
		{
			...
			if (RivalPrefix == "Rival ")
			{
				...
				if (StudentID == 12)
				{
					return SubtitleType.Rival2LostPhone;
				}
				return SubtitleType.LostPhone;
			}
	...
	private SubtitleType SplashSubtitleType
	{
		get
		{
			...
			if (RivalPrefix == "Rival ")
			{
				...
				if (StudentID == 12)
				{
					return SubtitleType.Rival2SplashReaction;
				}
				if (!Male)
				{
					return SubtitleType.SplashReaction;
	...
	public void Start()
	{
		...
		if (!Started)
		{
			...
			if (!StudentManager.Eighties)
			{
				...
				else if (StudentID == 12)
				{
					...
					OriginalIdleAnim = "f02_amaiIdle_00";
					OriginalWalkAnim = "f02_amaiWalk_00";
				...
				else if (StudentID == 23)
				{
					...
					CharacterAnimation["celebrate_00"].speed = 1.1f;
				}
				else if (StudentID == 24)
				{
					...
					CharacterAnimation["f02_celebrate_00"].speed = 1.2f;
				}
				else if (StudentID == 25)
				{
					...
					CharacterAnimation["f02_celebrate_00"].speed = 1.3f;
				}
				...
				else if (StudentID == 51)
				{
					...
					if (ClubGlobals.GetClubClosed(ClubType.LightMusic))
					{
						...
						{
							ScheduleBlock obj17 = ScheduleBlocks[2];
							obj17.destination = "Sulk";
							obj17.action = "Sulk";
							ScheduleBlock obj18 = ScheduleBlocks[4];
							obj18.destination = "Sulk";
							obj18.action = "Sulk";
							ScheduleBlock obj19 = ScheduleBlocks[7];
							obj19.destination = "Sulk";
							obj19.action = "Sulk";
							ScheduleBlock obj20 = ScheduleBlocks[8];
							obj20.destination = "Sulk";
							obj20.action = "Sulk";
						}
						else
						{
							ScheduleBlock obj21 = ScheduleBlocks[2];
							...
							obj23.action = "Sit";
							ScheduleBlock obj24 = ScheduleBlocks[8];
				...
				if ((StudentID == 6 || StudentID == 11) && DatingGlobals.SuitorProgress == 2)
				{
					...
					ScheduleBlock obj26 = ScheduleBlocks[2];
					...
					obj28.destination = "Locker";
					obj28.action = "Shoes";
					ScheduleBlock obj29 = ScheduleBlocks[8];
					obj29.destination = "Exit";
					obj29.action = "Exit";
				}
				if (Rival && !Slave && !StudentManager.MissionMode)
				{
					CheckIfWeNeedToPlaceBag();
					LovestruckTarget = 1;
				}
			}
			else
			{
				...
				if (!StudentManager.CustomMode)
				{
					...
					{
						ScheduleBlock obj32 = ScheduleBlocks[4];
						obj32.destination = "LunchSpot";
						obj32.action = "Eat";
					}
				}
			...
			if (StudentID == 1)
			{
				...
				if (StudentManager.MissionMode || GameGlobals.SenpaiMourning)
				{
					...
					base.gameObject.SetActive(value: false);
					Debug.Log("Deactivating student. 2");
				}
			}
		...
		if (StudentManager.Anniversary)
		{
			OriginalOriginalWalkAnim = GenderPrefix + "walkLively_00";
			OriginalWalkAnim = GenderPrefix + "walkLively_00";
			WalkAnim = GenderPrefix + "walkLively_00";
	...
	private void UpdateRoutine()
	{
		if (Routine)
		{
			...
			if (StalkerFleeing)
			{
				if (Actions[Phase] == StudentActionType.Stalk)
				{
					...
					{
						if (Vector3.Distance(Yandere.transform.position, StudentManager.FleeSpots[0].position) > 10f)
						{
					...
					{
						Debug.Log("Alright, we're more than 10 meters from the player, so we really don't need to be StalkerFleeing anymore.");
			...
			else if (Phase < ScheduleBlocks.Length - 1 && Clock.HourTime >= ScheduleBlocks[Phase].time && !InEvent && !Meeting && ClubActivityPhase < 16 && !Ragdoll.Zs.activeInHierarchy && !Dying && !Posing)
			{
				...
				if (Infatuated && CurrentAction == StudentActionType.Admire)
				{
					...
					else
					{
						...
						{
							Debug.Log("This code is running. 1");
							TargetDistance = 3.9f;
						}
					}
				}
				...
				if (MustChangeClothing && Schoolwear == 2 && CurrentAction != StudentActionType.Sunbathe)
				{
			...
			if (DistanceToDestination > TargetDistance)
			{
				...
				else if (Actions[Phase] == StudentActionType.Relax && Curious)
				{
					...
					{
						if (Crush == 16 && StudentManager.Students[Crush].transform.position.y > 1.3f && StudentManager.Students[Crush].transform.position.y < 1.4f)
						{
							TargetDistance = 6f;
						}
						else if (!StudentManager.Students[Crush].Drowned)
						{
							TargetDistance = 3.9f;
						}
						else
				...
				else if ((Actions[Phase] == StudentActionType.Stalk && !StalkerFleeing && DistanceToPlayer < 5f) || (Actions[Phase] == StudentActionType.Stalk && StudentManager.LockerRoomArea.bounds.Contains(Yandere.transform.position)))
				{
					if (Vector3.Distance(Yandere.transform.position, StudentManager.FleeSpots[0].position) > 10f || StudentManager.LockerRoomArea.bounds.Contains(Yandere.transform.position))
					{
						Pathfinding.target = StudentManager.FleeSpots[0];
						CurrentDestination = StudentManager.FleeSpots[0];
					}
					...
					Debug.Log(Name + " has decided to begin StalkerFleeing");
					Pathfinding.speed = 4f;
					StalkerFleeing = true;
				}
				if (StudentID == 10)
			...
			else
			{
				...
				if (CurrentDestination != null)
				{
					...
					else if (Infatuated && CurrentAction == StudentActionType.Admire)
					{
						if (DistanceToDestination > TargetDistance + 5f)
						{
							Pathfinding.speed = 5f;
				...
				if (!InEvent)
				{
					if (!Meeting)
					{
						if (!GoAway)
						{
							...
							else if (Actions[Phase] == StudentActionType.SitAndTakeNotes)
							{
								...
								else
								{
									...
									if (!EndSearch)
									{
										...
										{
											CharacterAnimation.CrossFade("f02_delinquentSit_00");
										}
									}
								}
							}
							...
							else if (Actions[Phase] == StudentActionType.SitAndEatBento)
							{
								...
								else if (!DiscCheck && Alarm < 100f)
								{
									...
									else if (Lethal)
									{
										...
										else if (StudentID == 11 && StudentManager.Students[1] != null && !StudentManager.Students[1].SenpaiWitnessingRivalDie && !StudentManager.Students[1].Emetic && !StudentManager.Students[1].MyBento.Emetic && Vector3.Distance(base.transform.position, StudentManager.Students[1].transform.position) < 2f)
										{
							...
							else if (Actions[Phase] == StudentActionType.Graffiti)
							{
								...
								if (GraffitiPhase == 0)
								{
									...
									CharacterAnimation.CrossFade(GenderPrefix + "bullyDesk_00");
									SmartPhone.SetActive(value: false);
									GraffitiPhase++;
								}
								else if (GraffitiPhase == 1)
								{
									if (CharacterAnimation[GenderPrefix + "bullyDesk_00"].time >= 8f)
									{
								...
								{
									if (CharacterAnimation[GenderPrefix + "bullyDesk_00"].time >= 9.66666f)
									{
										SpawnTimeRespectingAudioSource(BullyGiggles[UnityEngine.Random.Range(0, BullyGiggles.Length)]);
										GraffitiPhase++;
									}
								...
								{
									Debug.Log("We're stuck here.");
									if (CharacterAnimation[GenderPrefix + "bullyDesk_00"].time >= CharacterAnimation[GenderPrefix + "bullyDesk_00"].length)
									{
										GraffitiPhase++;
									}
								}
								...
								if (StudentManager.Students[StudentManager.VictimID] != null && StudentManager.Students[StudentManager.VictimID].Alive && !StudentManager.Students[StudentManager.VictimID].Tranquil && !StudentManager.Students[StudentManager.VictimID].InEvent)
								{
									...
									else
									{
										...
										if (BullyID == 1)
										{
											if (BullyPhase == 0)
											{
												...
												if (BullyTimer >= 10f && Vector3.Distance(base.transform.position, StudentManager.Students[1].transform.position) > 10f)
												{
													...
													CharacterAnimation.CrossFade(GenderPrefix + "bullyEraser_00");
													BullyPhase++;
												}
												else
												{
													...
													CharacterAnimation.CrossFade(GenderPrefix + "bullyGiggle_00");
												}
											}
											else if (BullyPhase == 1)
											{
												if (CharacterAnimation[GenderPrefix + "bullyEraser_00"].time >= 0.833333f)
												{
													BullyDust.Play();
													BullyPhase++;
												}
											...
											{
												if (CharacterAnimation[GenderPrefix + "bullyEraser_00"].time >= CharacterAnimation[GenderPrefix + "bullyEraser_00"].length)
												{
													SpawnTimeRespectingAudioSource(BullyLaughs[UnityEngine.Random.Range(0, BullyLaughs.Length)]);
													CharacterAnimation.CrossFade(GenderPrefix + "bullyLaugh_00");
													Scrubber.SetActive(value: false);
													Eraser.SetActive(value: false);
													BullyPhase++;
												}
											...
											{
												if (CharacterAnimation[GenderPrefix + "bullyLaugh_00"].time >= CharacterAnimation[GenderPrefix + "bullyLaugh_00"].length)
												{
													BullyPhase++;
												}
											}
										...
										else
										{
											...
											if (CharacterAnimation[GenderPrefix + "bullyLaugh_00"].time >= CharacterAnimation[GenderPrefix + "bullyLaugh_00"].length || StudentManager.Students[81].BullyPhase == 4 || BullyPhase == 4)
											{
									...
									else
									{
										...
										if (!FollowTarget.gameObject.activeInHierarchy || !FollowTarget.enabled)
										{
											...
											else
											{
												...
												base.gameObject.SetActive(value: false);
												Debug.Log("Deactivating student. 3");
											}
										}
										else
										{
							...
							else if (Actions[Phase] == StudentActionType.Sunbathe)
							{
								...
								if (!flag4)
								{
										...
										if (CharacterAnimation[StripAnim].time >= 1.5f)
										{
											...
											if (CharacterAnimation[StripAnim].time > CharacterAnimation[StripAnim].length)
											{
												...
												if (!StudentManager.CommunalLocker.SteamCountdown)
												{
													...
													if (CurrentDestination == null)
													{
														SunbathingID = StudentManager.SunbathingStudents;
														StudentManager.SunbatheSpots[StudentID] = StudentManager.CustomSunbatheSpots[SunbathingID];
														Destinations[Phase] = StudentManager.SunbatheSpots[StudentID];
														Pathfinding.target = StudentManager.SunbatheSpots[StudentID];
														CurrentDestination = StudentManager.SunbatheSpots[StudentID];
														StudentManager.SunbathingStudents++;
														Debug.Log(Name + "'s SunbathingID is now " + SunbathingID + ", and StudentManager.SunbathingStudents is now " + StudentManager.SunbathingStudents);
													}
													SunbathePhase++;
												}
											}
									...
									else if (SunbathePhase == 2)
									{
										...
										MyRenderer.updateWhenOffscreen = true;
										CharacterAnimation.cullingType = AnimationCullingType.AlwaysAnimate;
										CharacterAnimation.CrossFade(GenderPrefix + "sunbatheStart_00");
										SmartPhone.SetActive(value: false);
										if (CharacterAnimation[GenderPrefix + "sunbatheStart_00"].time >= CharacterAnimation[GenderPrefix + "sunbatheStart_00"].length)
										{
											MyController.radius = 0f;
											SunbathePhase++;
										}
									...
									else if (SunbathePhase == 3)
									{
										if (Sleepy)
										{
											if (!Blind)
											{
												...
												CharacterAnimation.CrossFade(GenderPrefix + "sunbatheSleep_00");
										...
										{
											CharacterAnimation.CrossFade(GenderPrefix + "sunbatheLoop_00");
										}
									}
								}
							}
							...
							else if (Actions[Phase] == StudentActionType.Admire)
							{
								...
								else
								{
									...
									{
										Debug.Log(Name + " is now firing StopFollowingGravureModel() from this place in the code.");
										StopFollowingGravureModel();
									}
									ScootAway();
								}
							...
							{
								if (StudentManager.BakeSale.ShutDown)
								{
									CharacterAnimation.CrossFade(ThinkAnim);
								}
								else if (BakeSalePhase == 0)
								{
									bool flag5 = false;
									if (StudentID != 12 && StudentManager.Students[12].BakeSalePhase == 0 && StudentManager.Students[12].ClubTimer == 0f)
									{
										flag5 = true;
									}
									if (StudentID == 12 || !flag5)
									{
										CharacterAnimation.CrossFade(PrepareFoodAnim);
										ClubTimer += Time.deltaTime;
										StopRotating = false;
									...
									else
									{
										...
										StopRotating = true;
										targetRotation = Quaternion.LookRotation(new Vector3(StudentManager.Students[12].transform.position.x, 0f, StudentManager.Students[12].transform.position.z) - base.transform.position);
										base.transform.rotation = Quaternion.Slerp(base.transform.rotation, targetRotation, 10f * Time.deltaTime);
									}
								}
								...
								else if (BakeSalePhase == 2)
								{
									...
									{
										StopRotating = true;
										if (StudentManager.BakeSale.CurrentCustomer != null && StudentManager.BakeSale.CustomerPresent)
										{
											if (StudentManager.BakeSale.CurrentCustomer.BakeSalePhase < 5)
											{
												if (Male)
												{
													CharacterAnimation.CrossFade("admire_00");
												}
												else
												{
													CharacterAnimation.CrossFade("f02_admire_01");
												}
											}
											else if (StudentManager.BakeSale.CurrentCustomer.BakeSalePhase == 5)
											{
												CharacterAnimation.CrossFade(GenderPrefix + "celebrate_00");
											}
											else if (!StudentManager.BakeSale.Poisoned)
											{
												if (Male)
											...
											AdmireAnim = AdmireAnims[UnityEngine.Random.Range(0, AdmireAnims.Length)];
											targetRotation = Quaternion.LookRotation(StudentManager.BakeSale.CurrentCustomer.transform.position - base.transform.position);
											base.transform.rotation = Quaternion.Slerp(base.transform.rotation, targetRotation, 10f * Time.deltaTime);
											SpeechLines.Stop();
										}
										else if (StudentID == 22)
										{
											CheckForPartnerTalkToPartner(22, 24);
										}
										else if (StudentID == 23)
										{
											CheckForPartnerTalkToPartner(23, 25);
										}
										else if (StudentID == 24)
										{
											CheckForPartnerTalkToPartner(24, 22);
										}
										else if (StudentID == 25)
										{
											CheckForPartnerTalkToPartner(25, 23);
										}
									}
									else
									{
						...
						if (Rival && CurrentDestination == StudentManager.RomanceSpot)
						{
							Meeting = true;
						}
					}
					else
					{
						if (MeetTimer == 0f)
						{
							...
							{
								Debug.Log("Now checking for what type of student is about to meet with the player.");
								...
								else if (StudentID == 5 || Persona == PersonaType.Fragile)
								{
									Debug.Log("It's a ''Fragile'' student.");
									Yandere.BullyPhotoCheck();
									if (Yandere.BullyPhoto)
									{
										Debug.Log("Player has a BullyPhoto. Let's fuckin' gooooo.");
										StudentManager.FragileOfferHelp.EventStudentID = StudentID;
						...
						if (MeetTimer > 60f)
						{
							if (!BakeSale)
							{
								if (!Male)
								{
									if (StudentID == 12)
									{
										Subtitle.UpdateLabel(SubtitleType.NoteReaction, 7, 5f);
									}
									else
									{
		...
		else
		{
					...
					if (!PinningDown)
					{
						...
						else
						{
							...
							if (base.transform.position.y < -10f)
							{
								...
								base.gameObject.SetActive(value: false);
								Debug.Log("Deactivating student. 4");
							}
							if (base.transform.position.y < -11f || (base.transform.position.y < -10f && base.transform.position.x > 13f))
							{
								...
								base.transform.position = new Vector3(base.transform.position.x, -100f, base.transform.position.z);
								base.gameObject.SetActive(value: false);
								Debug.Log("Deactivating student. 5");
							}
							...
							else
							{
								...
								else if (Persona == PersonaType.Heroic)
								{
									...
									else if (!Yandere.Attacking && !StudentManager.PinningDown && !Yandere.Shoved && !Yandere.Dumping && !Yandere.Dropping && !Yandere.BusyMurdering)
									{
										if (StudentID > 1)
										{
								...
								{
									if (!Yandere.Dumping && !Yandere.Attacking && !Yandere.Struggling && !Yandere.BusyMurdering)
									{
										bool flag7 = false;
										if (!StudentManager.ChallengeManager.InvincibleRaibaru && Yandere.PhysicalGrade + Yandere.Class.PhysicalBonus > 0)
										{
			...
			if (Distracting)
			{
				...
				{
					Debug.Log("Somehow, " + Name + "'s DistractionTarget became null.");
					Distracting = false;
				}
				...
				else
				{
					...
					if (DistanceToDestination < 5f || DistractionTarget.Leaving)
					{
						if (DistractionTarget.HelpOffered || DistractionTarget.InEvent || DistractionTarget.Talking || DistractionTarget.Following || DistractionTarget.TurnOffRadio || DistractionTarget.Splashed || DistractionTarget.Shoving || DistractionTarget.Spraying || DistractionTarget.FocusOnYandere || DistractionTarget.ShoeRemoval.enabled || DistractionTarget.Posing || DistractionTarget.ClubActivityPhase >= 16 || !DistractionTarget.enabled || DistractionTarget.Alarmed || DistractionTarget.Fleeing || DistractionTarget.Wet || DistractionTarget.EatingSnack || DistractionTarget.MyBento.Tampered || DistractionTarget.Meeting || DistractionTarget.Sedated || DistractionTarget.Sleepy || DistractionTarget.Guarding || DistractionTarget.InvestigatingBloodPool || DistractionTarget.ReturningMisplacedWeapon || StudentManager.LockerRoomArea.bounds.Contains(DistractionTarget.transform.position) || StudentManager.MaleLockerRoomArea.bounds.Contains(DistractionTarget.transform.position) || StudentManager.WestBathroomArea.bounds.Contains(DistractionTarget.transform.position) || StudentManager.EastBathroomArea.bounds.Contains(DistractionTarget.transform.position) || StudentManager.HeadmasterArea.bounds.Contains(DistractionTarget.transform.position) || (DistractionTarget.Actions[DistractionTarget.Phase] == StudentActionType.Bully && DistractionTarget.DistanceToDestination < 1f) || DistractionTarget.Leaving || DistractionTarget.CameraReacting || DistractionTarget.SentToLocker || (MyPlate != null && MyPlate.parent == RightHand && DistractionTarget.AlreadyFed) || DistractionTarget.AwareOfCorpse || (DistractionTarget.transform.position.y > 1f && DistractionTarget.CurrentAction == StudentActionType.Perform))
						{
							...
							if (StudentID == 47)
							{
								Debug.Log(Name + "'s Distraction Target is in a state which prevents them from being interacted with.");
							}
							CurrentDestination = Destinations[Phase];
							Pathfinding.target = Destinations[Phase];
							DistractionTarget.TargetedForDistraction = false;
						...
						else if (DistanceToDestination < TargetDistance)
						{
							...
							{
								if (StudentID > 1 && DistractionTarget.StudentID > 1 && Persona != PersonaType.Fragile && DistractionTarget.Persona != PersonaType.Fragile && ((Club != ClubType.Bully && DistractionTarget.Club == ClubType.Bully) || (Club == ClubType.Bully && DistractionTarget.Club != ClubType.Bully) || IsBully()))
								{
							...
							if (StudentID == StudentManager.SuitorID && StudentManager.DatingMinigame.SuitorAndRivalTalking && !Yandere.CanMove)
							{
								Debug.Log("Suitor knows that he's supposed to be talking to the rival right now.");
								if (DistractTimer <= 0f)
								{
									DistractTimer = 1f;
								}
			...
			if (Investigating)
			{
				...
				else if (InvestigationPhase == 1)
				{
					...
					if (DistanceToDestination > InvestigationDistance)
					{
						...
						{
							Debug.Log(Name + " has not moved more than .01f since the last time we checked their position.");
			...
			if (SentHome)
			{
				...
				if (base.transform.position.y < -11f || (base.transform.position.y < -10f && base.transform.position.x > 13f))
				{
					base.transform.position = new Vector3(base.transform.position.x, -100f, base.transform.position.z);
					base.gameObject.SetActive(value: false);
					Debug.Log("Deactivating student. 6");
				}
			}
			if (DramaticReaction)
			{
				...
				else if (SeekMedicinePhase == 1)
				{
					...
					if (DistanceToDestination < 1f)
					{
						...
						if (!studentScript4.Fleeing && !studentScript4.Guarding)
						{
							...
							studentScript4.RetrievingMedicine = true;
						...
						if (!flag12)
						{
							...
							if (CharacterAnimation[InspectBloodAnim].time >= CharacterAnimation[InspectBloodAnim].length || Persona == PersonaType.Strict)
							{
								...
								if (WitnessedWeapon)
								{
									...
									if (!WitnessedBloodyWeapon && StudentID > 1 && !flag16 && CurrentAction != StudentActionType.SitAndTakeNotes && Indoors && !flag14 && Club != ClubType.Delinquent && !flag13 && !flag15 && !BloodPool.GetComponent<WeaponScript>().Dangerous && BloodPool.GetComponent<WeaponScript>().Returner == null && BloodPool.GetComponent<WeaponScript>().Origin != null)
									{
			...
			if (SenpaiWitnessingRivalDie)
			{
				...
				if ((Yandere.Lifting || Yandere.Dragging) && Yandere.CurrentRagdoll == StudentManager.Students[StudentManager.RivalID].Ragdoll)
				{
					Distracted = false;
					Alarm = 200f;
				}
	...
	private void UpdateVisibleCelloLimbs()
	{
		ID = 1;
		while (ID < ClubManager.Container.BodyParts.Length && BloodPool == null)
		{
			if (ClubManager.Container.BodyParts[ID] != null)
			{
				Collider myCollider = ClubManager.Container.BodyParts[ID].MyCollider;
				Debug.Log(Name + " is checking to see if they are close enough to a limb to identify it.");
				Debug.DrawLine(Eyes.position, myCollider.transform.position, Color.red);
				if (CanSeeObject(myCollider.gameObject, myCollider.transform.position))
				{
					Debug.Log(Name + " saw the body part.");
				...
				{
					Debug.Log(Name + " could not see the body part.");
				}
			}
			ID++;
		}
		...
		if (!flag)
		{
			...
			if (!WitnessedMurder && !CheckingNote && !Shoving && !Slave && !Struggling && flag2 && !Drownable && !Fighting)
			{
				...
				if (VisibleCorpses.Count == 0 && !WitnessedCorpse)
				{
					...
					else if ((StudentID == StudentManager.CurrentID || (Persona == PersonaType.Strict && Fleeing)) && !Wet && !Guarding && !IgnoreBlood && !InvestigatingPossibleDeath && !Spraying && !Emetic && !Threatened && !Sedated && !Headache && !SentHome && !Slave && !Talking && !Confessing && !SentToLocker && !Meeting && !IgnoringThingsOnGround && !RetrievingMedicine && !StudentManager.KokonaTutorial)
					{
						if (BloodPool == null && StudentManager.Police.LimbParent.childCount > 0)
						{
							UpdateVisibleLimbs();
						}
						if (BloodPool == null && ClubManager.Container.NumberOfBodyParts > 0 && ClubManager.Container.Open)
						{
							UpdateVisibleCelloLimbs();
						}
						if (BloodPool == null)
						{
							bool flag3 = false;
									...
									if ((flag7 && CanSeeObject(Yandere.gameObject, headPosition3)) || AwareOfMurder)
									{
										...
										if (Yandere.Attacking || Yandere.Cauterizing || Yandere.Struggling || Yandere.WrappingCorpse || (WitnessedCorpse && Yandere.NearBodies > 0 && Yandere.Bloodiness + (float)Yandere.GloveBlood > 0f && !Yandere.Paint) || (WitnessedCorpse && Yandere.NearBodies > 0 && Yandere.Armed) || (WitnessedCorpse && Yandere.NearBodies > 0 && Yandere.Sanity < 66.66666f) || (Yandere.Carrying && !Yandere.CurrentRagdoll.Concealed) || (Yandere.Dragging && !Yandere.CurrentRagdoll.Concealed) || Yandere.MurderousActionTimer > 0f || (Guarding && Yandere.Bloodiness + (float)Yandere.GloveBlood > 0f && !Yandere.Paint) || (Guarding && Yandere.Armed && Yandere.EquippedWeapon.Dangerous) || (Guarding && Yandere.Armed && Yandere.EquippedWeapon.Suspicious) || (Guarding && Yandere.Sanity < 66.66666f) || (Guarding && Yandere.WearingRaincoat && !flag5) || (!IgnoringPettyActions && !StudentManager.CombatMinigame.Practice && Club == ClubType.Council && Yandere.DelinquentFighting && StudentManager.CombatMinigame.Path < 4 && !Yandere.SeenByAuthority) || (!StudentManager.CombatMinigame.Practice && Teacher && Yandere.DelinquentFighting && StudentManager.CombatMinigame.Path < 4 && !Yandere.SeenByAuthority) || (flag4 && Yandere.PickUp != null && Yandere.PickUp.Mop != null && Yandere.PickUp.Mop.Bloodiness > 0f) || (flag4 && Yandere.PickUp != null && Yandere.PickUp.BodyPart != null && !Yandere.PickUp.Garbage) || (Yandere.PickUp != null && Teacher && Yandere.PickUp.Clothing && Yandere.PickUp.Evidence) || (StudentManager.Atmosphere < 0.33333f && Yandere.Bloodiness + (float)Yandere.GloveBlood > 0f && Yandere.Armed) || (Fleeing && Yandere.Yakuza))
										{
											if (Yandere.Hungry || !Yandere.Egg)
											{
												...
												if (Persona == PersonaType.PhoneAddict && !Phoneless && Schoolwear > 0)
												{
										...
										else if (!Fleeing)
										{
											if (!Alarmed || CanStillNotice)
											{
												...
												{
													Debug.Log("Player was ''Rummagging'' or stealing, so Alarm should jump straight to 200.");
													Alarm = 200f;
												}
	...
	public void BecomeAlarmed()
	{
		...
		if (RetrievingMedicine)
		{
			Debug.Log("The character who just became alarmed was the nurse, and she was retrieving medicine at the moment that she became alarmed.");
			TargetStudent.SeekMedicinePhase = 5;
			RetrievingMedicine = false;
		}
				...
				else
				{
					...
					if (!Teacher)
					{
						if (Persona != PersonaType.Evil)
						{
							...
							{
								Debug.Log("This character just witnessed murder and is now reacting to it.");
								if (Rival && StudentID == 12)
								{
									Subtitle.UpdateLabel(SubtitleType.RivalMurderReaction, StudentID, 6.5f);
								}
								else
								{
		...
		if (!Threatened)
		{
			...
			else if (AlarmTimer > 1f && !Reacted)
			{
				...
				else if (Club != ClubType.Delinquent)
				{
					...
					{
						Debug.Log("Witnessed == StudentWitnessType.Insanity");
						if (Club == ClubType.Council)
						{
							if (StudentID == 86)
							{
								Subtitle.UpdateLabel(SubtitleType.CouncilNotice, 1, 5f);
							}
							else if (StudentID == 87)
							{
								Subtitle.UpdateLabel(SubtitleType.CouncilNotice, 2, 5f);
							}
							else if (StudentID == 88)
							{
								Subtitle.UpdateLabel(SubtitleType.CouncilNotice, 3, 5f);
							}
							else if (StudentID == 89)
							{
								Subtitle.UpdateLabel(SubtitleType.CouncilNotice, 4, 5f);
							}
						}
						else if (!StudentManager.Eighties && StudentID == 12)
						{
							Subtitle.Speaker = this;
							Subtitle.CustomText = "...uh...what exactly are you...trying to do...?...that's kind of...weird...";
							Subtitle.CustomClip = Talk.AmaiReact;
							Subtitle.UpdateLabel(SubtitleType.Custom, 0, 10f);
						}
						else
						{
				...
				else if (WitnessedLimb || WitnessedWeapon || WitnessedBloodPool || WitnessedBloodyWeapon)
				{
					Subtitle.UpdateLabel(SubtitleType.LimbReaction, 0, 5f);
				}
				else
	...
	private void UpdateSplashed()
	{
		...
		if (LightSwitch == null)
		{
			...
			{
				Subtitle.Speaker = this;
				Subtitle.UpdateLabel(SplashSubtitleType, 8, 5f);
			}
			else if (Gas)
			{
				Subtitle.Speaker = this;
	...
	private void UpdateMisc()
	{
		...
		if (!Fleeing)
		{
			if (base.transform.position.z < -100f)
			{
				...
				{
					base.gameObject.SetActive(value: false);
					Debug.Log("Deactivating student. 7");
				}
				return;
			}
	...
	private void LateUpdate()
	{
		...
		if (CurrentDestination == null)
		{
			if (!StudentManager.Eighties && !StudentManager.AlphabetKillerChallenge && StudentID == 7 && Clock.HourTime < 7.9f)
			{
				base.gameObject.SetActive(value: false);
				Debug.Log("Disabling Student from here.");
			}
			else
			{
				Debug.Log("Uh-oh! For some reason, Student #" + StudentID + ", " + Name + ", had a null CurrentDestination.");
	...
	public void FixNullDestination()
	{
		...
		if (Destinations[Phase] == null)
		{
			Debug.Log("Problem. " + Name + "'s Destinations[Phase] is null. Why is that?");
	...
	public void AttackReaction()
	{
		...
		if (SeekingMedicine)
		{
			...
			if (studentScript != null && studentScript.RetrievingMedicine)
			{
	...
	private void WitnessMurder()
	{
		...
		Pathfinding.canSearch = false;
		Pathfinding.canMove = false;
		if (!Phoneless && Schoolwear > 0 && (Persona == PersonaType.PhoneAddict || Sleuthing))
		{
			SmartPhone.SetActive(value: true);
		}
		if (SmartPhone.activeInHierarchy)
	...
	private void PersonaReaction()
	{
		...
		if ((Persona == PersonaType.PhoneAddict && Phoneless) || (Persona == PersonaType.PhoneAddict && Schoolwear == 0) || (Persona == PersonaType.SocialButterfly && Phoneless))
		{
			Debug.Log(Name + " was going to report the crime with a phone, but they don't have a phone right now, so they are switching to the Loner Persona.");
			Persona = PersonaType.Loner;
		}
		...
		if (!DoNotMourn && InCouple && Corpse != null && Corpse.Student.StudentID == PartnerID)
		{
		...
		else if (Persona == PersonaType.Lovestruck)
		{
			...
			else
			{
				...
				if (!StudentManager.Students[LovestruckTarget].WitnessedMurder)
				{
					...
					StudentManager.Students[LovestruckTarget].CanTalk = false;
					TargetDistance = 1f;
					ReportPhase = 1;
				}
				else
	...
	public void GetDestinations()
	{
		...
		for (ID = 1; ID < JSON.Students[StudentID].ScheduleBlocks.Length; ID++)
		{
			...
			else if (scheduleBlock.destination == "Sunbathe")
			{
				if (Male)
				{
					Destinations[ID] = StudentManager.StrippingPositions[0];
				}
				else
				{
			...
			else if (scheduleBlock.destination == "BakeSale")
			{
				if (StudentManager.BakeSale.ShutDown)
				{
					Debug.Log("The Bake Sale was shut down! Student #" + StudentID + " should change their destination to one of the ''Ponder Spots''.");
					Destinations[ID] = StudentManager.BakeSalePonderSpots[StudentID];
				}
				else if (BakeSalePhase == 0)
				{
			...
			else if (scheduleBlock.destination == "AfterRivalWitnessSpot")
			{
				if (!AfterRivalWitness)
				{
			...
			else if (scheduleBlock.destination == "BakeSalePrepSpot")
			{
				Destinations[ID] = StudentManager.BakeSalePrepSpots[StudentID];
			}
			...
			else if (scheduleBlock.action == "Socialize")
			{
				if (StudentID == 26)
				{
					Debug.Log("One of " + Name + "'s Actions was set to ''Socializing.'' If this character is having problems, this might be the cause.");
				}
			...
			else if (scheduleBlock.action == "PrepareFoodForever")
			{
	...
	public void AddOutlineToHair()
	{
		...
		if (Teacher && StudentManager.Eighties && EightiesTeacherAttacher != null && EightiesTeacherAttacher.GetComponent<RiggedAccessoryAttacher>().newRenderer != null)
		{
			...
			EightiesTeacherRenderer.materials[0].mainTexture = Cosmetic.FaceTexture;
	...
	private void BecomeTeacher()
	{
		...
		if (StudentID == 94)
		{
			Spine.GetChild(0).GetChild(0).GetChild(1)
				.GetComponent<DynamicBoneCollider>()
				.m_Radius = 0.05f;
			Spine.GetChild(0).GetChild(0).GetChild(4)
				.GetComponent<DynamicBoneCollider>()
				.m_Radius = 0.05f;
		}
	}
	...
	public void BecomeRagdoll()
	{
		...
		if (StudentManager.QualityManager.CensorCorpses)
		{
			Debug.Log("The game knows that this student's corpse should be censored.");
			Debug.Log("Alive is: " + Alive);
			for (int i = 0; i < MyRenderer.materials.Length; i++)
			{
				StudentManager.QualityManager.ApplyMatrixSettings(MyRenderer.materials[i], StudentManager.QualityManager.MatrixTexture);
			}
	...
	public void StopMeeting()
	{
		...
		if (BakeSale && StudentManager.BakeSale.CurrentCustomer == this)
		{
			Debug.Log(Name + " was the Bake Sale's ''CurrentStudent'' when this code fired.");
	...
	public void UpdateAnimLayers()
	{
		...
		if (Club == ClubType.Bully)
		{
			...
			{
				CharacterAnimation[GenderPrefix + "bullyLaugh_00"].speed = 0.9f + (float)BullyID * 0.1f;
			}
		}
		else if (Club == ClubType.Delinquent)
		{
		...
		if (Tranquil)
		{
			...
			CharacterAnimation.Play(GenderPrefix + "carryDisposeB_00");
			CharacterAnimation[GenderPrefix + "carryDisposeB_00"].time = CharacterAnimation[GenderPrefix + "carryDisposeB_00"].length;
		}
		if (Ragdoll.enabled)
		{
			...
			if (Ragdoll.Concealed)
			{
				if (Ragdoll.InsideIncinerator)
				{
					...
					base.gameObject.SetActive(value: false);
					Debug.Log("Deactivating student. 8");
				}
			}
			else
			{
		...
		if (Ragdoll.InsideIncinerator)
		{
			Debug.Log("This character was wrapped in a garbage bag and inside of an incinerator at the point in time when UpdateAnimLayers() was called.");
			base.gameObject.SetActive(value: false);
			Debug.Log("Deactivating student. 9");
		}
	}
	...
	{
		if (CharacterAnimation != null && CharacterAnimation[SocialSitAnim].weight != 0f)
		{
	...
	public void CheckIfWeNeedToPlaceBag()
	{
		if (StudentID > 11 && StudentID < 21)
		{
			...
			{
				Debug.Log(Name + " will not be adding ''PlaceBag'' to their routine.");
			}
		}
	}

	public void CheckForPartnerTalkToPartner(int selfID, int targetID)
	{
		StudentScript studentScript = StudentManager.Students[targetID];
		if (studentScript != null && Vector3.Distance(base.transform.position, studentScript.transform.position) < 1f)
		{
			CharacterAnimation.CrossFade(RandomAnim);
			if (CharacterAnimation[RandomAnim].time >= CharacterAnimation[RandomAnim].length)
			{
			...
			SpeechLines.Play();
			targetRotation = Quaternion.LookRotation(studentScript.transform.position - base.transform.position);
			base.transform.rotation = Quaternion.Slerp(base.transform.rotation, targetRotation, 10f * Time.deltaTime);
		}
		else
		{
	...
	public bool IsBully()
	{
		return Array.Exists(OriginalActions, (StudentActionType action) => action == StudentActionType.Bully || action == StudentActionType.Graffiti);
	}

	public void OsanaBecomesLoner()
	{
		Debug.Log("Updating Osana's routine to make her patrol around school.");
		ScheduleBlock obj = ScheduleBlocks[2];
		obj.destination = "Patrol";
		obj.action = "Patrol";
		ScheduleBlock obj2 = ScheduleBlocks[7];
	...
	private Transform FindDeepChild(Transform parent, string name)
	{
		foreach (Transform item in parent)
		{
			...
			Transform transform2 = FindDeepChild(item, name);
			if (transform2 != null)
			{
				return transform2;
			}
